...
const responseGetFilteredJobPostings = (db, req, res) => {
    if (req.body === undefined) {
        ...
        if (filters.job_posting_frequencies && filters.job_posting_frequencies.length > 0) {
            const frequencies = filters.job_posting_frequencies.map(frequency => `'${frequency}'`).join(', ');
